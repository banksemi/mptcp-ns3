...
int main (int argc, char *argv[]) {
  ...
  // default route
    ...
    //stack.SysctlSet(nodes, ".net.ipv4.tcp_sack", "0");
    ...
    Simulator::Schedule(Seconds(12), &ChangeRTT, (_switch) % 2 , StringValue("100Mbps"), StringValue(std::to_string(set_rtt1) + "ms")); // start UE movement
    Simulator::Schedule(Seconds(12), &ChangeRTT, (_switch + 1) % 2, StringValue("100Mbps"), StringValue(std::to_string(set_rtt2) + "ms")); // start UE movement
    _switch++;    
    Simulator::Schedule(Seconds(52), &ChangeRTT, (_switch) % 2 , StringValue("100Mbps"), StringValue(std::to_string(set_rtt1) + "ms")); // start UE movement
    Simulator::Schedule(Seconds(52), &ChangeRTT, (_switch + 1) % 2, StringValue("100Mbps"), StringValue(std::to_string(set_rtt2) + "ms")); // start UE movement
    _switch++;

    if (rtt_change) {
        for(int i = 32; i<62; i += 1) {
            if (pacing == false) {
